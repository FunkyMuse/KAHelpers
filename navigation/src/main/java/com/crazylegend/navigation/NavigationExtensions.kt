...
package com.crazylegend.navigation
...
fun <T> Fragment.setNavigationResultPrevious(result: T, key: String = "result") {
    findNavController().previousBackStackEntry?.savedStateHandle?.set(key, result)
}

fun <T> Fragment.setNavigationResultCurrent(result: T, key: String = "result") {
    findNavController().currentBackStackEntry?.savedStateHandle?.set(key, result)
}

private fun runDelayed(delay: Long, timeUnit: TimeUnit = TimeUnit.MILLISECONDS, action: () -> Unit) {
    Handler(Looper.getMainLooper()).postDelayed(action, timeUnit.toMillis(delay))
}

inline fun tryOrIgnore(action: () -> Unit) = try {
    action()
} catch (e: Exception) {
